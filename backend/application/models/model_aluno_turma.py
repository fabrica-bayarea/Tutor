...
class AlunoTurma(db.Model):
    ...
    aluno_id = db.Column(db.UUID(as_uuid=True), db.ForeignKey('usuario.id'), nullable=False, primary_key=True)
    turma_id = db.Column(db.UUID(as_uuid=True), db.ForeignKey('turmas.id'), nullable=False, primary_key=True)

    aluno = db.relationship('Usuario', back_populates='turmas_matriculadas')
